...
class LocalServer(object):
  ...
  def LocalKmlSearchHandler(self, handler):
    ...
    if "displayKeys" in list(handler.request.arguments.keys()):
      key = handler.request.arguments["displayKeys"][0]
      if key in list(handler.request.arguments.keys()):
        try:
  ...
  def LocalOneBoxKmlSearchHandler(self, handler):
    ...
    if "GEPlacesPlugin" in list(self.search_services_.keys()):
